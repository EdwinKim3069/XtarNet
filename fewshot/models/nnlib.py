...
def compute_euc(weight, keys):
    ...
    log_prob: [N, K] logits.
    """
    weight = tf.expand_dims(weight, 0)  # [1, K, D]
    keys = tf.expand_dims(keys, 1)  # [N, 1, D]
    # [N, K]
    neg_dist = -tf.reduce_sum(tf.square(keys - weight), [-1])
    return neg_dist


def compute_logits_cosine(weight, keys):
  """Computes the logits of being in one cluster, squared Euclidean.
  Args:
    weight: [K, D] Cluster center representation.
    keys: [N, D] Data representation.
  Returns:
    log_prob: [N, K] logits.
  """
  weight = tf.expand_dims(weight, 0)  # [B, K, D]
  weight = tf.expand_dims(weight, 1)  # [B, 1, K, D]
  keys = tf.expand_dims(keys, 0)  # [B, N, D]
  keys = tf.expand_dims(keys, 2)  # [B, N, 1, D]
  # [B, N, K]
  logits = tf.reduce_sum(keys * weight, [-1])
  logits = logits / tf.sqrt(tf.reduce_sum(tf.square(keys), [-1])) / tf.sqrt(
      tf.reduce_sum(tf.square(weight), [-1]))
  return logits[0]


def nullspace_signalling(average_key, novel_w, h_size=512, eps=1e-6):
    M_t = average_key
    W_rho = novel_w
    power_W_rho = tf.sqrt(tf.reduce_sum(W_rho * W_rho, axis=1, keepdims=True))
    W_rho = W_rho / (power_W_rho + eps)
    power_M = tf.sqrt(tf.reduce_sum(M_t * M_t, axis=1, keepdims=True))
    M_tmp = M_t / (power_M + eps)
    null = W_rho - M_tmp
    z = nullspace_gpu(null, h_size - 5)
    return z
