...
from fewshot.models.nnlib import compute_euc
log = get_logger()


class XtarNetModelBase(object):
  """Base class for XtarNet models."""
  ...
  def build_task_a(self, x, y, is_training, ext_wts=None):
    ...
    h_a, h_meta = self._run_backbone(x, is_training=is_training, ext_wts=ext_wts)
    self._h_a = h_a
  ...
  def build_task_a_grad(self):
    ...
    print('build_task_a_grad done')
    return grads_and_vars_a

  def _run_backbone(self, x, ext_wts=None, reuse=None, is_training=True, MetaCNN=False):
  ...
  def _aggregate_grads_and_vars(self, grads_and_vars_list, weights=None):
    print('_aggregate_grads_and_vars')
    ...
    print('_aggregate_grads_and_vars done')
    return result
  ...
  def _apply_transfer_loss(self, fast_weights, reuse=None, **kwargs):
    ...
    print('_apply_transfer_loss done')
  ...
  def get_meta_weights(self):
    ...
    var_list += self.get_transfer_loss_weights(name='encoder_g_gamma')
    var_list += self.get_transfer_loss_weights(name='encoder_g_beta')
    var_list += self.get_transfer_loss_weights(name='encoder_h')  # including 'encoder_h_pre'
  ...
  def eval_step_a(self, sess, task_a_data):
    ...
    # logits_a = sess.run([self.prediction_a], feed_dict=fdict)
    # print(logits_a)
